...
//        AppEventsLogger.activateApp(this);
    ...
    private void initServerConfigs(){
        LibConfigs awsConfigs = new LibConfigs.Builder()
                .setConfigFileName(A.getServerFileName())
                .setDomain(A.getServerHelperUrl())
                .setDefFbAdSpace("Nothing")
                .setDebugTag("ApiTag")
                .setShowLogs(BuildConfig.DEBUG)
                .setIEventLoggerInterface(new AwsLogger.IEventLogger() {
                    @Override
                    public void logEvent(String event) {}
                })
                .build();

        AwsApp.initialize(getApplicationContext(), awsConfigs);
    }
